...
In our multimedia application can define an abstraction for rendering content without concerning itself with the specifics of each rendering device. Each rendering device, such as a screen, printer, or projector, has its own implementation class encapsulating the device-specific rendering logic.
